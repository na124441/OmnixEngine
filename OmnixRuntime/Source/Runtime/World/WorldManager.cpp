...
namespace Omnix
{
                            ...
                            for (const auto& obj : objects)
                            {
                                if (obj)
                                {
                                    ...
                                    if (ent != INVALID_ENTITY)
                                    {
                                        ...
                                        // Add ZoneMembershipComponent
                                        eng::runtime::ZoneMembershipComponent zmc;
                                        zmc.zoneUUIDHigh = zone.zoneUUIDHigh;
                                        zmc.zoneUUIDLow = zone.zoneUUIDLow;
                                        coordinator->AddComponent<eng::runtime::ZoneMembershipComponent>(ent, zmc);
                                    }
                                }
                            }
                            delete zoneScene;
    ...
    void WorldManager::Update(eng::runtime::RuntimeContext& context, float deltaTime)
    {
        ...
        if (context.ecs) {
            ...
            if (world) {
                if (auto playerControllerSys = world->getCoordinator().GetSystemByName("class eng::runtime::PlayerControllerSystem")) {
    ...
    std::vector<uint32_t> WorldManager::QueryEntitiesInZone(uint64_t high, uint64_t low, Coordinator& coordinator) const
    {
        ...
        for (Entity ent : coordinator.GetActiveEntities())
        {
            if (coordinator.IsEntityAlive(ent))
            {
                ...
                if (sig.test(zmcType))
                {
                    ...
                    if (zmc.zoneUUIDHigh == high && zmc.zoneUUIDLow == low)
                    {
    ...
    std::vector<uint32_t> WorldManager::QueryActiveZoneEntities(Coordinator& coordinator) const
    {
        return QueryEntitiesInZone(m_ActiveZoneUUIDHigh, m_ActiveZoneUUIDLow, coordinator);
    }

    std::vector<WorldZone> WorldManager::QueryNearbyZones(const Vec3& position, float radius) const
    {
        std::vector<WorldZone> result;
        for (const auto& zone : m_LoadedZones)
        {
            float dx = std::max(zone.bounds.min.x - position.x, std::max(0.0f, position.x - zone.bounds.max.x));
            float dy = std::max(zone.bounds.min.y - position.y, std::max(0.0f, position.y - zone.bounds.max.y));
            float dz = std::max(zone.bounds.min.z - position.z, std::max(0.0f, position.z - zone.bounds.max.z));
    ...
    std::vector<uint32_t> WorldManager::QueryNeighboringZoneEntities(uint64_t high, uint64_t low, Coordinator& coordinator) const
    {
        std::vector<uint32_t> result;
        const WorldZone* targetZone = nullptr;
        for (const auto& zone : m_LoadedZones)
        {
            if (zone.zoneUUIDHigh == high && zone.zoneUUIDLow == low)
            {
                targetZone = &zone;
                break;
            }
        ...
        if (targetZone)
        {
            for (const auto& neighbor : targetZone->neighbors)
            {
                auto neighborEntities = QueryEntitiesInZone(neighbor.zoneUUIDHigh, neighbor.zoneUUIDLow, coordinator);
                result.insert(result.end(), neighborEntities.begin(), neighborEntities.end());
            }
        }
        return result;
    }

    void WorldManager::PrintZoneMembershipDiagnostics(Coordinator& coordinator) const
    {
        LOG_INFO("--- Zone Membership Diagnostics ---");
        auto zmcType = coordinator.GetComponentType<eng::runtime::ZoneMembershipComponent>();
        std::unordered_map<std::string, int> zoneEntityCounts;
        int orphanedCount = 0;
        int totalWithComponent = 0;
            ...
            if (coordinator.IsEntityAlive(ent))
            {
                ...
                if (sig.test(zmcType))
                {
                    ...
                    for (const auto& zone : m_LoadedZones)
                    {
                        ...
                        {
                            zoneEntityCounts[zone.zoneName]++;
                            matched = true;
                            break;
                        }
                    }
        ...
        LOG_INFO("Total Entities with ZoneMembershipComponent: %d", totalWithComponent);
        for (const auto& pair : zoneEntityCounts)
        {
            LOG_INFO("  Zone '%s': %d entities", pair.first.c_str(), pair.second);
        }
        if (orphanedCount > 0)
        {
            LOG_WARN("  Orphaned/Outside-bounds Entities: %d", orphanedCount);
        }
        else
        {
            LOG_INFO("  No orphaned entities detected.");
        }
        LOG_INFO("-----------------------------------");
    }
}
