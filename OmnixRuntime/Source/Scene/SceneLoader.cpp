    ...
    if (objectData.HasMember("components") && objectData["components"].IsArray()) {
        for (const auto& comp : objectData["components"].GetArray()) {
            if (comp.HasMember("type") && comp["type"].IsString()) {
                ...
                } else if (compType == "GroundSection") {
                    eng::runtime::GroundSectionComponent gsc;
                    if (comp.HasMember("zoneUUIDHigh") && comp["zoneUUIDHigh"].IsUint64()) gsc.zoneUUIDHigh = comp["zoneUUIDHigh"].GetUint64();
                    if (comp.HasMember("zoneUUIDLow") && comp["zoneUUIDLow"].IsUint64()) gsc.zoneUUIDLow = comp["zoneUUIDLow"].GetUint64();
                    if (comp.HasMember("meshAssetPath") && comp["meshAssetPath"].IsString()) gsc.meshAssetPath = comp["meshAssetPath"].GetString();
                    if (comp.HasMember("materialAssetPath") && comp["materialAssetPath"].IsString()) gsc.materialAssetPath = comp["materialAssetPath"].GetString();
                    if (comp.HasMember("collisionAssetPath") && comp["collisionAssetPath"].IsString()) gsc.collisionAssetPath = comp["collisionAssetPath"].GetString();
                    
                    if (comp.HasMember("boundsMin") && comp["boundsMin"].IsObject()) {
                        const auto& minObj = comp["boundsMin"];
                        if (minObj.HasMember("x") && minObj["x"].IsNumber()) gsc.boundsMin.x = minObj["x"].GetFloat();
                        if (minObj.HasMember("y") && minObj["y"].IsNumber()) gsc.boundsMin.y = minObj["y"].GetFloat();
                        if (minObj.HasMember("z") && minObj["z"].IsNumber()) gsc.boundsMin.z = minObj["z"].GetFloat();
                    }
                    if (comp.HasMember("boundsMax") && comp["boundsMax"].IsObject()) {
                        const auto& maxObj = comp["boundsMax"];
                        if (maxObj.HasMember("x") && maxObj["x"].IsNumber()) gsc.boundsMax.x = maxObj["x"].GetFloat();
                        if (maxObj.HasMember("y") && maxObj["y"].IsNumber()) gsc.boundsMax.y = maxObj["y"].GetFloat();
                        if (maxObj.HasMember("z") && maxObj["z"].IsNumber()) gsc.boundsMax.z = maxObj["z"].GetFloat();
                    }
                    if (comp.HasMember("debugDraw") && comp["debugDraw"].IsBool()) gsc.debugDraw = comp["debugDraw"].GetBool();
                    sceneObject->SetGroundSection(gsc);
