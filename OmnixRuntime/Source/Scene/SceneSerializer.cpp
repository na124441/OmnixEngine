...
bool SceneSerializer::SaveScene(Scene* scene, const std::string& filePath) {
    ...
    for (const auto& object : scene->GetAllSceneObjects()) {
        ...
        if (object->m_HasGroundSection) {
            Value comp(kObjectType);
            comp.AddMember("type", "GroundSection", allocator);
            comp.AddMember("zoneUUIDHigh", object->m_GroundSection.zoneUUIDHigh, allocator);
            comp.AddMember("zoneUUIDLow", object->m_GroundSection.zoneUUIDLow, allocator);
            comp.AddMember("meshAssetPath", Value(object->m_GroundSection.meshAssetPath.c_str(), allocator).Move(), allocator);
            comp.AddMember("materialAssetPath", Value(object->m_GroundSection.materialAssetPath.c_str(), allocator).Move(), allocator);
            comp.AddMember("collisionAssetPath", Value(object->m_GroundSection.collisionAssetPath.c_str(), allocator).Move(), allocator);
            
            Value boundsMinVal(kObjectType);
            boundsMinVal.AddMember("x", object->m_GroundSection.boundsMin.x, allocator);
            boundsMinVal.AddMember("y", object->m_GroundSection.boundsMin.y, allocator);
            boundsMinVal.AddMember("z", object->m_GroundSection.boundsMin.z, allocator);
            comp.AddMember("boundsMin", boundsMinVal, allocator);

            Value boundsMaxVal(kObjectType);
            boundsMaxVal.AddMember("x", object->m_GroundSection.boundsMax.x, allocator);
            boundsMaxVal.AddMember("y", object->m_GroundSection.boundsMax.y, allocator);
            boundsMaxVal.AddMember("z", object->m_GroundSection.boundsMax.z, allocator);
            comp.AddMember("boundsMax", boundsMaxVal, allocator);

            comp.AddMember("debugDraw", object->m_GroundSection.debugDraw, allocator);
            components.PushBack(comp, allocator);
        }
