...
namespace eng::core {
    ...
    void ModuleManager::RegisterModule(std::shared_ptr<IModule> module) {
        ...
        LOG_INFO("[ModuleManager] Registered module: %s", name.c_str());
    }
    ...
    bool ModuleManager::InitializeModules(RuntimeContext& context) {
        ...
        if (!BuildTopologicalOrder()) {
            LOG_ERROR("[ModuleManager] Failed to build dependency order. Initialization aborted.");
            return false;
        }

        LOG_INFO("[ModuleManager] Initializing modules in dependency-sorted order:");
        for (auto& module : m_SortedModules) {
            LOG_INFO("  -> %s", module->GetName().c_str());
            if (!module->Init(context)) {
                LOG_ERROR("[ModuleManager] Module %s failed to initialize!", module->GetName().c_str());
    ...
    void ModuleManager::ShutdownModules() {
        ...
        LOG_INFO("[ModuleManager] Shutting down modules in reverse dependency order:");
        for (auto it = m_SortedModules.rbegin(); it != m_SortedModules.rend(); ++it) {
            LOG_INFO("  <- %s", (*it)->GetName().c_str());
            (*it)->Shutdown();
        }
    ...
    bool ModuleManager::BuildTopologicalOrder() {
        ...
        for (const auto& [name, module] : m_Modules) {
            ...
            for (const auto& dep : deps) {
                ...
                if (m_Modules.find(dep) == m_Modules.end()) {
                    LOG_ERROR("[ModuleManager] Module '%s' depends on '%s', but '%s' is not registered!",
        ...
        if (m_SortedModules.size() != m_Modules.size()) {
            LOG_ERROR("[ModuleManager] Circular dependency detected in module configuration!");
            return false;
        }
