...
#include "Core/Engine/RuntimeContext.h"
...
namespace eng::core {
    ...
    bool EngineRuntime::Initialize(int argc, char* argv[]) {
        ...
        bool testContainers = false;
        bool testMath = false;
        bool testErrors = false;
        bool testJobs = false;
        bool testScheduler = false;
        bool testFileUtils = false;
        bool testWindow = false;
        bool testMonitors = false;
        bool testInput = false;
        bool testPlatformFS = false;
        bool testDylib = false;
        bool testClipboard = false;
        ...
        for (int i = 1; i < argc; ++i) {
            if (argv[i]) {
                ...
                if (arg == "--game") {
                    ...
                    testGolden = true;
                } else if (arg == "--test-containers") {
                    testContainers = true;
                } else if (arg == "--test-math") {
                    testMath = true;
                } else if (arg == "--test-errors") {
                    testErrors = true;
                } else if (arg == "--test-jobs") {
                    testJobs = true;
                } else if (arg == "--test-scheduler") {
                    testScheduler = true;
                } else if (arg == "--test-fileutils") {
                    testFileUtils = true;
                } else if (arg == "--test-window") {
                    testWindow = true;
                } else if (arg == "--test-monitors") {
                    testMonitors = true;
                } else if (arg == "--test-input") {
                    testInput = true;
                } else if (arg == "--test-platformfs") {
                    testPlatformFS = true;
                } else if (arg == "--test-dylib") {
                    testDylib = true;
                } else if (arg == "--test-clipboard") {
                    testClipboard = true;
                } else if (arg == "--test-timer") {
                    testTimer = true;
                } else if (arg == "--test-threads") {
                    testThreads = true;
                } else if (arg == "--test-layers") {
                    testLayers = true;
                } else if (arg == "--editor") {
                    m_Context.mode = RuntimeMode::Editor;
                }
        ...
        if (testContainers) {
            // Run custom container validation tests
            bool success = eng::runtime::RunContainerTests();
            ExitTest(success);
        }

        if (testMath) {
            // Run custom math library validation tests
            bool success = eng::runtime::RunMathTests();
            ExitTest(success);
        }

        if (testErrors) {
            // Run custom error/result validation tests
            eng::test::RunErrorTests();
            ExitTest(true);
        }

        if (testJobs) {
            // Run custom job system validation tests
            eng::runtime::RunJobTests();
            ExitTest(true);
        }

        if (testScheduler) {
            // Run custom system scheduler & task graph validation tests
            eng::runtime::RunSchedulerTests();
            ExitTest(true);
        }

        if (testFileUtils) {
            // Run custom file utilities & safe filesystem validation tests
            eng::runtime::RunFileTests();
            ExitTest(true);
        }

        if (testWindow) {
            // Run custom window management validation tests
            eng::runtime::RunWindowTests();
            ExitTest(true);
        }

        if (testMonitors) {
            // Run custom monitor enumeration validation tests
            eng::runtime::RunMonitorTests();
            ExitTest(true);
        }

        if (testInput) {
            // Run custom input backend validation tests
            eng::runtime::RunInputTests();
            ExitTest(true);
        }

        if (testPlatformFS) {
            // Run custom platform filesystem validation tests
            eng::runtime::RunPlatformFileSystemTests();
            ExitTest(true);
        }

        if (testDylib) {
            // Run custom dynamic library loader validation tests
            eng::runtime::RunDynamicLibraryTests();
            ExitTest(true);
        }

        if (testClipboard) {
            // Run custom clipboard validation tests
            eng::runtime::RunClipboardTests();
            ExitTest(true);
        }

        if (testTimer) {
            // Run custom high-resolution timer validation tests
            eng::runtime::RunTimerTests();
            ExitTest(true);
        }

        if (testThreads) {
            // Run custom platform thread validation tests
            eng::runtime::RunThreadTests();
            ExitTest(true);
        }

        if (testLayers) {
            // Run custom LayerStack validation tests
            eng::runtime::RunLayerTests();
            ExitTest(true);
        }
        ...
        m_TimeManager = std::make_unique<eng::core::TimeManager>();
        m_TimeManager->Initialize();

        m_ModuleManager = std::make_unique<eng::core::ModuleManager>();
        m_ServiceRegistry = std::make_unique<ServiceRegistry>();
        m_PluginManager = std::make_unique<PluginManager>();
        m_EventBus = std::make_unique<eng::runtime::EventBus>();
        m_LayerStack = std::make_unique<eng::core::LayerStack>();
        ...
        if (m_Context.mode != RuntimeMode::Editor) {
            ...
            m_InputThread = eng::platform::Thread("InputWorker", -1, &EngineRuntime::InputThreadWorker, this);
        }
        ...
        // 4. Renderer (EngineLoop) Initialization
        ...
        if (auto* loop = dynamic_cast<EngineLoop*>(m_Renderer.get())) {
            loop->SetExternalWorld(static_cast<World*>(m_ECS.get()));
        }
        ...
        m_Context.layerStack = m_LayerStack.get();
    ...
    void EngineRuntime::Run() {
        ...
        while (IsRunning()) {
            ...
            float smoothDt = static_cast<float>(dt);
            if (m_TimeManager) {
                m_TimeManager->Update(smoothDt);
                smoothDt = m_TimeManager->GetDeltaTime();
            }
            ...
            auto updateStart = Clock::now();
            {
                ...
                if (m_LayerStack) {
                    m_LayerStack->OnUpdate(smoothDt);
                }
            }
            ...
            auto renderStart = Clock::now();
            {
                ...
                if (m_LayerStack) {
                    try {
                        ...
                        LOG_ERROR("[Runtime] Exception in LayerStack::OnRender: %s", e.what());
                        throw;
                    }
                }
            }
            ...
            eng::runtime::LogFrameDiagnostics(m_Timing.deltaTime);
    ...
    void EngineRuntime::Shutdown() {
        ...
        // Gameplay Event Bus & Event System Shutdown
        if (m_GameplayEventBus) {
            m_GameplayEventBus.reset();
            TrackDeallocation("GameplayEvents", sizeof(GameplayEventBus));
        }
